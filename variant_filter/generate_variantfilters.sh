...
do
	...
	do
		zcat $i | awk -F"\t" -vins=${ins} -vi=$i 'BEGIN{OFS="\t"}$2 ~ /[0-9]+/{print ins,i,$9;}' | sort | uniq >> ${output_filter}/udn_filter_FORMAT.txt &
		zcat $i | awk -F"\t" -vins=${ins} -vi=$i 'BEGIN{OFS="\t"}$2 ~ /[0-9]+/{print ins,i,$1;}' | sort | uniq >> ${output_filter}/udn_contig.txt &
		prefix=$(basename ${i});
		bash ${parse_vcf_cmd} $i > ${output_filter}/${prefix}.filter.txt &
	done
done
